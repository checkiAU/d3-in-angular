  ...
  @Output() dateChanged = new EventEmitter<any>();
	...
	clicked(d, p, e) {
		...
		p.svg
			...
			.on("end", p.drillDown(translate[0], translate[1], scale, d.abbrev, p.type, p.scale, p.metric, p.date)); // updated for d3 v4
	}

	drillDown(x, y, scale, state, type, mapScale, metric, date) {
		...
		this.router.navigateByUrl("/counties/" + state + "/" + type + "/" + mapScale + "/" + metric + "/" + date);
	}
